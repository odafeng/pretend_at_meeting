# -*- coding: utf-8 -*-
"""
pretend_at_meeting — turn a folder of conference/session recordings into:
  1) a per-video summary for every talk            (summarize)
  2) one topic-chaptered master summary            (synthesize)
  3) a .pptx deck told as if YOU attended the meeting and are sharing what you learned (slides)
...
def find_ffmpeg(explicit=None):
    ...
    if ff and fp:
        return ff, fp
    sys.exit("❌ ffmpeg/ffprobe not found. Put them in ./ffmpeg/bin, set FFMPEG_DIR, or add to PATH.")
...
def make_chunks(ffmpeg, video, wdir):
    ...
    cmd = [ffmpeg, "-y", "-i", str(video), "-vn", "-f", "segment",
           "-segment_time", str(CHUNK_SECONDS), "-acodec", "pcm_s16le",
...
def stage_summarize(args):
    ...
    for i, v in enumerate(videos, 1):
        ...
        if tfile.exists() and tfile.stat().st_size > 50:
            ...
            try:
                transcript = transcribe_video(client, ffmpeg, v, d["audio"] / stem, args.transcribe_model)
            except Exception as e:
        ...
        try:
            summ = summarize_one(client, args.summary_model, args.meeting, args.lang, stem, transcript)
        except Exception as e:
...
def stage_synthesize(args):
    ...
    def do():
        return client.chat.completions.create(model=args.synth_model, messages=[{"role": "user", "content": prompt}])
...
# --------------------------------------------------------------------------- #
#  Stage 3: slides (.pptx) as if you attended
# --------------------------------------------------------------------------- #
SLIDES_PROMPT = """你要幫一位「親自參加了『{meeting}』（會期：{dates}）」的講者，製作一份「回來向同事分享所學」的簡報大綱。
語氣是第一人稱、像真的去開會回來分享（例如「這次大會我最大的收穫是…」「現場討論最熱烈的是…」）。
語言：繁體中文，專有名詞保留原文。
...
{{
  "title": "簡報主標題",
  "subtitle": "副標（含會議名與日期）",
  "intro_bullets": ["開場 3-5 點：我為什麼去、整體印象、這份分享的架構"],
  "sections": [
    {{
      "chapter": "章節標題（一個主題）",
      "slides": [
        {{"heading": "投影片標題", "bullets": ["重點 1","重點 2","重點 3-6"], "notes": "口語講稿備註(1-3句)"}}
      ]
    }}
  ],
...
- notes 寫成第一人稱口語講稿，補充 slide 上沒寫的細節或臨床觀點。
- 內容要忠實反映大摘要，但用「我在現場學到」的敘事包裝。
...
def build_pptx(deck, path, meeting=""):
    ...
    def rect(slide, x, y, w, h, color, line=None):
        ...
        shp.fill.solid(); shp.fill.fore_color.rgb = C(color) if isinstance(color, str) else color
        if line is None:
    ...
    def footer(slide):
        ...
        r = tf.paragraphs[0].add_run(); r.text = meeting
        set_run(r, 9, "muted")
        tf2 = textbox(slide, 11.6, 7.08, 1.03, 0.3)
        tf2.paragraphs[0].alignment = PP_ALIGN.RIGHT
        r2 = tf2.paragraphs[0].add_run(); r2.text = str(page["n"])
        set_run(r2, 9, "muted")
    ...
    def add_title(title, subtitle):
        ...
        rc.text = "整理自會議官方場次錄影 · A first-person field report"
    ...
    def add_divider(num, title):
        ...
        r0 = tf0.paragraphs[0].add_run(); r0.text = f"CHAPTER {num:02d}"
    ...
    def add_content(heading, bullets, notes=None, dark=False):
        ...
        if dark:
            ...
            hr = htf.paragraphs[0].add_run(); hr.text = heading
    ...
    if deck.get("intro_bullets"):
        add_content("開場 ｜ Why I went & what's ahead", deck["intro_bullets"])
...
def add_common(p, need_videos=False):
    ...
    p.add_argument("--reuse-outline", action="store_true",
                   help="(slides) rebuild the deck from existing slides_outline.json without calling GPT")


def main():
    ap = argparse.ArgumentParser(description="pretend_at_meeting — summarize a folder of talks and present as if you attended.")
    sub = ap.add_subparsers(dest="cmd", required=True)
    for name, need_v in [("summarize", True), ("synthesize", False), ("slides", False), ("all", True)]:
